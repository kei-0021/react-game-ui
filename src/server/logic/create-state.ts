...
export function createState(roomId: RoomId, param: GameParam): RoomState {
  ...
  const state: RoomState = {
    ...
    pieces: Object.fromEntries(
      Object.entries(param.initialPieces || {}).filter(([_, p]) => p.ownerId !== ('player' as any)),
    ),
    exploredCells: [],
...
export function createPlayer(param: GameParam, state: RoomState, playerName: string, socketId: string): Player {
  ...
  if (param.initialPieces) {
    const templates = Object.entries(param.initialPieces).filter(([_, p]) => p.ownerId === ('player' as any));
