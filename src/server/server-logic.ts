...
function initializeRoom(roomId: RoomId, param: GameParam): RoomState {
  ...
  let draggable: Record<DraggableId, DraggableData> = {};
  if (param.draggable) draggable = param.draggable;
...
export function initGameServer(io: Server, options: GameServerOptions) {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('board:movable-range', ({ roomId, boardId, playerId, moveRange }: BoardMovableRangeData) => {
      const state = activeRooms.get(roomId);
      if (!state) return;
      const param = gameParams[state.gameId];
      const roomManager = new RoomManager(io, param, state);
    ...
    // コンテキストメニュー：最前面
    socket.on('object:bring-to-front', ({ roomId, objectId, type }: ObjectBringToFrontData) => {
      const state = activeRooms.get(roomId);
      if (!state) return;
      const param = gameParams[state.gameId];
      const roomManager = new RoomManager(io, param, state);
