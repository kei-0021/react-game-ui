...
export const TokenDisplayContent = React.memo(({ token }) => {
    // 画像がある場合
    if (token.imageSrc) {
        return (_jsx("div", { className: styles.contentWrapper, style: { backgroundColor: token.color || '#4f4848ff' }, children: _jsx("img", { src: token.imageSrc, alt: token.name, className: styles.image }) }));
    }
    // 画像がない場合
