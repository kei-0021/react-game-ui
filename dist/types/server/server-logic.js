...
export function initGameServer(io, options = {}) {
    ...
    const emitPlayerUpdate = (roomId) => {
        const roomState = activeRooms.get(roomId);
        if (roomState)
            io.to(roomId).emit('players:update', roomState.initRoomState.players);
    };
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('room:join', async ({ roomId, playerName, gamePresetId }) => {
            ...
            server_log('game', roomState.gameName, roomId, `ターン更新 (Player: ${roomState.initRoomState.players[roomState.currentTurnIndex]?.name}, Round: ${roomState.currentRoundIndex})`);
